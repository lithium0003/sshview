...
struct ConsoleView: View {
    ...
    @State var screenBuffer: [UInt8] = []
    @State var remainStdIn: [UInt8] = []

    @StateObject var screen = consoleScreen()
    ...
    func stdout(_ data: ArraySlice<UInt8>) {
        if data.isEmpty, !screenBuffer.isEmpty {
            DispatchQueue.main.async {
                screenView = AnyView(renderScreen())
            }
        }
        else {
            DispatchQueue.main.async {
    ...
    var body: some View {
        VStack {
            HStack {
                Text(windowTitle).font(.headline.monospaced())
                Spacer()
            }
            GeometryReader { geometry in
                screenView
                    ...
                    .onAppear() {
                        ...
                        let newSize = screen.setSize(size: geometry.size)
                    ...
                    .onChange(of: geometry.size, perform: { newValue in
                        ...
                        let newSize = screen.setSize(size: newValue)
                        handler.screeSizeChane?(newSize.width, newSize.height)
                    })
                    .onChange(of: screen.fontSize) { newValue in
                        print("fontsize:\(screen.fontSize)")
                        let newSize = screen.setSize(size: geometry.size)
                        handler.screeSizeChane?(newSize.width, newSize.height)
                    }
            }
        }
        .background(InvisibleTextViewWrapper(isFirstResponder: $screen.showingKeyboad, textInputMode: $textInputMode, console: screen))
        .onTapGesture(count: 2) {
            if screen.showingKeyboad {
                inHndler([9])
            }
        }
        .onTapGesture(count: 1) {
            screen.showingKeyboad.toggle()
        }
        .onAppear() {
            handler.stdOutFcn = stdout(_:)
            handler.stdInFcn = stdin
            screen.stdinHandler = inHndler(_:)
        }
        .onChange(of: textInputMode) { value in
            if value {
                screen.showingKeyboad = false
                textInput = ""
            }
        }
...
struct ConsoleView_Previews: PreviewProvider {
    ...
    @State static var windowTitle = "test"
    @State static var screenWidth = 80
    @State static var screenHeight = 24

    static var previews: some View {
        ConsoleView(isActive: $isActive, handler: handler, windowTitle: $windowTitle)
    }
}
